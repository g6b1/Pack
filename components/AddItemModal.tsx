...
const AddItemModal: React.FC<AddItemModalProps> = ({ onClose, onAdd, onUpdate, onAddMultiple, editingItem, activeTheme }) => {
  const [customLabel, setCustomLabel] = useState(editingItem?.label || '');
  const [customSubtext, setCustomSubtext] = useState(editingItem?.subtext || '');
  const [selectedEmoji, setSelectedEmoji] = useState(editingItem?.icon || '📌');
    ...
    if (customLabel.trim()) {
      if (editingItem) {
        onUpdate(editingItem.id, {
          label: customLabel,
          subtext: customSubtext.trim() || 'Updated just now',
          icon: selectedEmoji
        });
      } else {
        ...
        onAdd({
          label: customLabel,
          subtext: customSubtext.trim() || 'Added just now',
          icon: selectedEmoji,
          color: randomColor
        });
      }
  ...
  return (
    <div className="absolute inset-0 z-[100] flex flex-col justify-end">
      ...
      <motion.div
        ...
        className="relative w-full h-[88%] bg-white rounded-t-[44px] shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden"
      >
        ...
        <div className="px-8 flex items-center justify-between pt-4 pb-6 shrink-0">
          <h2 className="text-2xl font-[900] tracking-tighter text-slate-900 leading-none">
            {editingItem ? 'Edit Item' : 'Add to Pack'}
          </h2>
          <button 
            onClick={onClose}
        ...
        <div className="px-8 flex-1 overflow-y-auto no-scrollbar pb-32">
          {!editingItem && (
            <>
              ...
              <div className="mb-10">
                <h3 className={`text-[11px] font-black uppercase tracking-[0.25em] mb-6 opacity-40 ${activeTheme.textColor}`}>Pack Templates</h3>
              ...
              <div className="mb-10">
                <h3 className={`text-[11px] font-black uppercase tracking-[0.25em] mb-6 opacity-40 ${activeTheme.textColor}`}>Quick Items</h3>
          ...
          <div className="mt-4">
             <h3 className={`text-[11px] font-black uppercase tracking-[0.25em] mb-6 opacity-40 ${activeTheme.textColor}`}>
               {editingItem ? 'Item Details' : 'Custom'}
             </h3>
             <div className="bg-slate-50 rounded-[32px] p-6 ring-1 ring-slate-100">
                ...
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-2">
                    <label className={`text-[10px] font-black uppercase tracking-widest opacity-60 ${activeTheme.textColor}`}>Description</label>
                    <span className={`text-[10px] font-black ${customSubtext.length >= 60 ? 'text-rose-500' : 'text-slate-300'}`}>
                      {customSubtext.length}/60
                    </span>
                  </div>
                  <input 
                    value={customSubtext}
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleSave()}
                    className="w-full bg-white rounded-2xl px-4 h-12 border border-slate-100 focus:ring-2 focus:ring-black/5 focus:border-slate-200 text-[14px] font-semibold text-slate-600 placeholder:text-slate-200" 
                ...
                >
                  {editingItem ? 'Save Changes' : 'Add to Pack'}
                </button>
              </div>
          </div>
        </div>
